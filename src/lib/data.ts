...
function rssEpisodeToEpisode(ep: RSSEpisode): Episode {
  ...
  const staticEpisode = override as Episode | null
  const staticConcepts = (staticEpisode?.concepts as string[] | undefined) ?? []
  const staticChapters = (staticEpisode?.chapters as string[] | undefined) ?? []
  ...
  return {
    ...
    logo: staticEpisode?.logo || ep.logo || EPISODE_ART_FALLBACK,
